...
work_path="d:\\Research\\ResearchMethods\\Python-Book-Package\\Raschka-MachineLearningPyTorchScikitLearn-2022-Point45\\machine-learning-book-main\\ch12"
...
#for å avoid restarting kernel after matplotlib in line 378-390
...
#Image.open will get shape of (height,weight,channel)
...
#this loop added by Wilson, showing that shape of image data
#after preprocessing image shape is like (3, 80, 120) - (depth, img_height,img_width)
#An image for data analysis in imshow is with sequence (height, width,depth)
#ax.imshow() : The first two dimensions (M, N, 3) define 
#the rows and columns of the image, and third dimension is channel.
#so need transpose to (1,2,0) in the next looping for plotting
for i, example in enumerate(image_dataset):
    print('Image shape: ', example[0].shape)
    print('Image label: ', example[1])


#imshow is height, width
...
# 
#     - You can try setting `download=True` below. If this results 
# in a `BadZipfile` error, we recommend downloading 
#the `img_align_celeba.zip` file manually from
# http://mmlab.ie.cuhk.edu.hk/projects/CelebA.html. 
#In the Google Drive folder, you can find it under
# the `Img` folder as shown below:
...
# After completing steps 1-3 above, please ensure you have the following 
#files in your `./celeba` subfolder, and the files are non-empty 
#(that is, they have similar file sizes as shown below):
